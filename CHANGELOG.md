...
- `download` works on the seat's real Chrome. The seat attaches to a Chrome it launched as a person would, so Playwright managed no downloads. Chrome saved each file to the person's Downloads folder, and the verb waited for an event that never came. Every attach and every command now points Chrome's downloads at the seat's own `downloads` directory (`Browser.setDownloadBehavior`). That override lasts only while its CDP session stays attached, so one session is kept for the life of the attach. The verb waits for a new finished file there (not `.crdownload`), moves it into the run's artifacts, and prints its path, as before. A timeout names the directory, and names the partial file when one is still arriving. On macOS, Chrome still creates and at once deletes an empty `.com.google.Chrome.*` temp file in the person's Downloads folder on each download. A profile's default download directory preference does not move it.
